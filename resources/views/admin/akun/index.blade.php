...
@section('title', 'Master Akun Perkiraan')
    ...
    <div class="col-xs-12">
      ...
      <div class="box">
        <div class="box-header">
          <h3 class="box-title">Data Akun Perkiraan</h3>
        </div>
...
$(function(){
  ...
  $('#modal-form form').validator().on('submit', function(e){
    if(!e.isDefaultPrevented()){
      var id = $('#id').val();
      if(save_method == "add") url = "{{ route('akun.store') }}";
      else url = "akun/"+id;
      $.ajax({
        url : url,
        type : "POST",
...
function editForm(id){
  ...
  $.ajax({
    ...
    success : function(data){
      ...
      $('#id').val(data.akun_id);
      $('#akun_kode').val(data.akun_kode);
      $('#akun_nama').val(data.akun_nama);
      $('#akun_saldo_normal').val(data.akun_saldo_normal);
      $('#akun_id_parent').val(data.akun_id_parent).trigger('change');
    },
    error : function(){
